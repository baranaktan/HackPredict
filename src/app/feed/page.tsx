                ...
                >
                  ...
                  <div className="absolute right-2 bottom-28 z-50 flex flex-col items-center gap-8 pointer-events-auto">
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center text-xl transition-all pointer-events-auto ${liked[stream.id || 0] ? 'text-blue-600 scale-110' : 'text-white hover:text-blue-500'}`}
                    ...
                    <div className="relative">
                      {isCommentsOpen && (
                        <div
                          ...
                          className="absolute right-0 bottom-10 w-72 max-w-[90vw] z-50 bg-cream/95 border-4 border-black rounded-lg p-4 shadow-window-pixel max-h-64 overflow-y-auto mt-2 scrollbar-thin scrollbar-thumb-blue-300 scrollbar-track-blue-100 flex flex-col"
                          style={{ minWidth: 260 }}
                        >
                          {/* Close (X) button */}
                          <button
                            className="absolute top-2 right-2 w-7 h-7 flex items-center justify-center rounded-full bg-blue-600 text-white font-bold border-2 border-black shadow hover:bg-yellow-400 hover:text-blue-500 transition-colors z-10"
                            onClick={() => handleToggleComments(stream.id || 0)}
                                ...
                                <div
                                  className={`px-3 py-2 rounded-2xl border-2 border-black max-w-[80%] text-xs break-words shadow-window-pixel ${
                                    cidx % 2 === 0
                                      ...
                                      : 'bg-yellow-100 text-blue-600 rounded-br-none'
                                  }`}
                                >
                                  <span className="font-bold text-blue-600 mr-1">{c.name}:</span> {c.text}
                                </div>
                              </li>
                            ))}
                          </ul>
                          ...
                          >
                            <input
                              ...
                              className="flex-1 px-2 py-1 rounded-lg border-2 border-black text-xs bg-white focus:outline-none focus:ring-2 focus:ring-blue-600"
                              maxLength={120}
                            />
                            <button
                              type="submit"
                              className="px-3 py-1 rounded-2xl border-2 border-black bg-blue-600 text-white text-xs shadow-window-pixel hover:bg-yellow-400 hover:text-blue-500 transition-colors"
                            >
                              Send
                            </button>
                          </form>
