  ...
  return (
    <section className="max-w-lg mx-auto my-8">
      ...
      <div className="mb-4 flex items-center gap-2">
        <label htmlFor="market-select" className="text-xs font-bold text-blue-800">Select Market:</label>
        <select
          id="market-select"
      ...
      <div className="border-4 border-black bg-yellow-300 shadow-window-pixel">
        ...
        {/* table */}
        <div className="bg-blue-100 p-4 overflow-x-auto">
          {isLoading ? (
            <div className="text-center py-8">
              <div className="animate-spin w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full mx-auto mb-4"></div>
              <p className="text-sm text-blue-800">Loading livestreams...</p>
            </div>
          ) : error ? (
            <div className="text-center py-8">
              ...
              <button 
                ...
                className="mt-2 bg-blue-600 text-white px-3 py-1 border-2 border-black text-xs hover:bg-blue-700"
              >
                Retry
              </button>
            </div>
            ...
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="text-blue-800 border-b-2 border-black">
                ...
                {leaderboardData.map((entry) => (
                  <tr
                    ...
                    className={`border-b border-black ${
                      entry.rank <= 3 ? "bg-yellow-200/60" : entry.rank % 2 ? "bg-blue-200/40" : ""
                    }`}
                  >
          ...
          {!isLoading && !error && leaderboardData.length === 0 && (
            <div className="text-center py-8">
              <p className="text-sm text-blue-800">No livestreams found</p>
              <p className="text-xs text-blue-600 mt-1">Start streaming to see them on the leaderboard!</p>
            </div>
          )}
        </div>
      </div>
