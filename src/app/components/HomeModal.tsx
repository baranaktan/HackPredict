...
export const HomeModalContent: React.FC = () => (
  <div className="flex flex-col items-center justify-center min-w-[420px] min-h-[340px] max-w-2xl mx-auto">
      ...
      <div className="flex-1 flex flex-col items-center md:items-end text-right max-w-xs">
        <div className="w-full border-2 border-black bg-periwinkle p-2 rounded shadow-window-pixel">
          <div className="bg-cream p-2 rounded text-left">
            ...
            <div className="text-blue-600 text-[11px] sm:text-xs leading-tight">Watch a livestream, connect your wallet, and bet on outcomes—all on-chain and transparent.</div>
          </div>
        </div>
      </div>
      {/* Logo and Title */}
      <div className="flex flex-col items-center flex-shrink-0 px-2">
        <Image src="https://res.cloudinary.com/storagemanagementcontainer/image/upload/v1751729735/live-stakes-icon_cfc7t8.png" alt="HackPredict Logo" width={160} height={160} className="mb-2 hue-rotate-[85deg] saturate-150" />
        <span className="text-2xl sm:text-3xl text-blue-600 tracking-widest leading-tight font-extrabold" style={{textShadow: '2px 2px 0 #000, 0 2px 0 #000, 2px 0 0 #000'}}>
          HackPredict
        </span>
      </div>
      {/* Why join */}
    ...
    <div className="w-full max-w-xl text-xs sm:text-sm leading-relaxed text-center mb-6 text-blue-800">
      HackPredict is the world&apos;s first <b>AI-powered prediction market platform</b> built for hackathons and live events.
    </div>
    {/* Connect Wallet at the bottom */}
    <div className="mt-auto w-full flex justify-center">
      <ConnectWallet style="header" color="green" />
    </div>
  </div>
);
...
const HomeModal: React.FC<HomeModalProps> = ({ isOpen, onClose, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="border-4 border-black bg-blue-100 shadow-window-pixel max-w-2xl w-full mx-4">
        {/* Title bar */}
        <div className="flex items-center justify-between bg-blue-600 text-yellow-50 px-3 py-1 border-b-4 border-black">
